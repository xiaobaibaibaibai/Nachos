...
main(int argc, char **argv)
{
    ...
    List<char*>* userProgNameList = new List<char*>();
    ...
    List<char*>* copyUnixFileNames = new List<char*>();
    List<char*>* copyNachosFileNames = new List<char*>();
    ...
    for (i = 1; i < argc; i++) {
        ...
        userProgNameList->Append(argv[i + 1]);
	    i++;
	}
        ...
        copyUnixFileNames->Append(argv[i + 1]);
        copyNachosFileNames->Append(argv[i + 2]);
	    i += 2;
	}
    ...
    if (copyUnixFileNames->NumInList() != 0 && copyNachosFileNames->NumInList() != 0) {
        ListIterator<char*>* itU = new ListIterator<char*>(copyUnixFileNames);
        ListIterator<char*>* itN = new ListIterator<char*>(copyNachosFileNames);
        while (!itU->IsDone() && !itN->IsDone()) {
            char *copyUnixFileName = itU->Item();
            char *copyNachosFileName = itN->Item();
    ...
    if(userProgNameList->NumInList()!=0){
        bzero(kernel->machine->mainMemory, MemorySize);
        ListIterator<char*>* it = new ListIterator<char*>(userProgNameList);
        while(!it->IsDone()){
            char* userProgName = it->Item();
            Thread* t = new Thread(userProgName);
            t->Fork((VoidFunctionPtr)RunUserProg, (void*)userProgName);
            it->Next();
        }
