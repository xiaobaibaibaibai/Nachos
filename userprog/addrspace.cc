...
AddrSpace::Load(char *fileName) 
{
    ...
    if (executable == NULL) {
	    cerr << "In load(), Unable to open file " << fileName << "\n";
...
// Note: this code assumes that virtual address = physical address
    ...
    if (noffH.code.size > 0) {
        ...
        for (int i = 0; i < numPages; i++) {
	        pageTable[i].virtualPage = kernel->swapSpace_counter++;	// for now, virt page # != phys page #
	        // pageTable[i].physicalPage = -1;
            pageTable[i].physicalPage = pageTable[i].virtualPage;
            ...
            //Write the buffer into swapSpace file
            // kernel->swapSpace->WriteAt(buffer, PageSize, pageTable[i].virtualPage * PageSize);
            bcopy(buffer, &kernel->machine->mainMemory[pageTable[i].virtualPage * PageSize ], PageSize);
        }
    }
